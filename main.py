...
from PointCloudCreator import create_point_cloud, load_pcd, load_pcd_with_mesh
...
def auto_regis_vis(auto_regis=True, use_accel=True):
    inputter = KeyInputter(1, 1)
    key_dict = register_keys(inputter, 0.01)
    source, scene, translation, mesh_id = load_pcd_with_mesh('sampleDatas/RoomScan.ply', 'sampleDatas/mesh.obj')

    if auto_regis:
        pos_dict = define_qr_pos(
            glob.glob("sampleDatas/RoomScan/frame*.json"),
            source,
            mesh_scene=scene,
            trans=translation
        )

        pos = np.array(pos_dict['135322064678'])
    ...
    with RealSense(serial_num='135322064678') as real_sense:
        with RealSenseVis(key_dict=key_dict) as rs_vis:
            ac_sensor = None
            if use_accel:
                ac_sensor = AccelSensor("COM3", 9600)
                ac_sensor.open_serial()
            ...
            while not inputter.is_exit:
                ...
                if auto_regis and ac_sensor is not None:
            ...
            if ac_sensor is not None:
                ac_sensor.close_serial()


if __name__ == '__main__':
    auto_regis_vis(use_accel=False)
